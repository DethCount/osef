class Function2dRenderer {
    ...
    render(context, prevContext) {
        if (this.funUI.viewState == 'hidden') {
            this.clear();
            return this;
        }
    ...
    renderSegment(args1, args2) {
        ...
        if (args1) {
            val1 = this.funUI.fun.evaluate(args1);
            if (!isNaN(val1)) {
                p1 = this.funUI.space.applyTransformation(
                    this.funUI.space.mergeContextAndVector(args1, new Vector2(args1.x, val1))
                );
            }
        }

        if (args2) {
            val2 = this.funUI.fun.evaluate(args2);
            if (!isNaN(val2)) {
                p2 = this.funUI.space.applyTransformation(
                    this.funUI.space.mergeContextAndVector(args2, new Vector2(args2.x, val2))
                );
            }
        }
